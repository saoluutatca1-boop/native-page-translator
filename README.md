...
- **Cache bản dịch dùng chung** — sống trong service worker, dùng chung cho mọi tab và còn nguyên sau khi trình duyệt dọn service worker. Mở lại cùng một trang, mở tab thứ hai cùng site, hay bấm Gốc rồi dịch lại đều **không tốn thêm request nào**. Chỉ lưu **mã băm** của đoạn gốc kèm bản dịch — không lưu nội dung trang. Tắt/xoá trong **Cài đặt → Khác → Cache bản dịch**.
...
- **Cache bản dịch (v4.3)**: lưu trong `chrome.storage.local` của extension, **chỉ gồm mã băm của đoạn gốc + bản dịch** — không lưu URL, không lưu văn bản gốc, không gửi đi đâu. Tắt bằng toggle trong **Cài đặt → Khác**, xoá sạch bằng nút "Xoá cache bản dịch", và gỡ extension là mất theo. File **xuất cài đặt** không bao giờ kèm API key lẫn cache.
